...
*/
    ...
    if($bank=='الأهلي' || $bank=="الاهلي" || $bank=="الأهلى" || $bank=="الاهلى" )
    {
    ...
    elseif($bank=='الراجحي'|| $bank=='الراجحى')
    {
          ...
          for($i=0;$i<count($operationsx);$i++)
        {
            ...
            if(
                ...
                isset($check_donor_exist['amount_of_withdrawal']) &&
                filter_var($operationsx[$i][0], FILTER_VALIDATE_FLOAT)   !=0 &&
                $check_donor_exist['amount_of_withdrawal'] == filter_var($operationsx[$i][0], FILTER_VALIDATE_FLOAT) &&
                explode('/', $operationsx[$i][5])[0]==$check_donor_exist['day_of_withdraw']
            ){ //success
            ...
            elseif(
                ...
                $total_another_amount_money +=filter_var($operationsx[$i][0], FILTER_VALIDATE_FLOAT);
            ...
            else if(filter_var($operationsx[$i][0], FILTER_VALIDATE_FLOAT) !=0)
            {
                $total_another_amount_money +=filter_var($operationsx[$i][0], FILTER_VALIDATE_FLOAT);
                $temp_array=array(
                    'type'=>'أخري',
    ...
    public function storeAutoConfirm(Request $request)
    {
        ...
        if($bank=='الأهلي' || $bank=="الاهلي" || $bank=="الأهلى" || $bank=="الاهلى" )
        {
            ...
            for($i=0;$i<count($request->successeded_operations);$i++)
            {
                ...
                $this->recordSystemFees($establishment, $operation);
                if($check_donor_exist->establishment->send_sms && $check_donor_exist->success_sms)
                {
        ...
        elseif($bank=='الراجحي'|| $bank=='الراجحى')
        {
            for($i=0;$i<count($request->successeded_operations);$i++)
            {
                ...
                $operation=Operation::create([
                    ...
                    'state'=>$request->successeded_operations[$i]['type'],
                    'type'=>"auto",
                    'amount'=>$request->successeded_operations[$i]['operation'][0],
                    'excel_sheet_id'=>$request->excel_sheet->id,
                    'success'=>true,
                    'summary'=>(string)json_encode($request->successeded_operations[$i]['operation']),
                    'operation_date'=>date('Y-m-d h:i:s'),
            ...
            for($i=0;$i<count($request->failed_operations);$i++)
            {
                $check_donor_exist=Donor::where('id',$request->failed_operations[$i][0])->get()->first();
            ...
            for($i=0;$i<count($request->another_operations);$i++)
            {
                ...
                $operation=Operation::create([
                    ...
                    'state'=>$request->another_operations[$i]['type'],
                    'type'=>"auto",
                    'amount'=>$request->another_operations[$i]['operation'][0],
                    'excel_sheet_id'=>$request->excel_sheet->id,
                    'success'=>false,
                    'summary'=>(string)json_encode($request->another_operations[$i]['operation']),
                    'operation_date'=>date('Y-m-d h:i:s'),
