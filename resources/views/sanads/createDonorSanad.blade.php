            ...
            <div class="panel-heading  col-12" style="padding: 0px; ">
                <div class="panel-body col-12 row " style=" background: #2381c6;color: #fff;padding:  20px 5px; ">
                    <div class="col-6 " style="padding: 4px">
                        <span class="fa fa-clock" style="color: var(--orange-light-color);margin-right: 10px;padding-left: 20px;font-size: 22px;"></span>  سنادات تسجيل متبرعين
                    </div>
                </div>
            </div>
                ...
                <div class="col-12 real-content row " style="opacity: 0;padding: 30px 0px;position: relative;">
                    ...
                    <table id="myTable" class="table table-striped table-bordered col-xs-12" style="width:100%;padding: 0px;">
                        ...
                        <tr>
                            ...
                            <th>تاريخ تسجيل المتبرع</th>
                            <th>تاريخ بداية الأستقطاع</th>
                            <th>تاريخ نهاية الأستقطاع</th>
                            <th>الفرع</th>
                            <th>عملية</th>
                        </tr>
                        </thead>
                        ...
                        @foreach($sanads as $sanad)
                            ...
                            <tr>
                                ...
                                <td>{{$sanad->amount}}</td>
                                <td>{{$sanad->registration_date}}</td>
                                <td>{{$sanad->withdrawal_start_date}}</td>
                                <td>{{$sanad->withdrawal_end_date}}</td>
                                <td>{{$sanad->branch_id ? $sanad->branch->name : "الفرع الرئيسي"}}</td>


                                <td style="max-width: 100px">
                                    <a href="{{ route('sanads.createDonorSanadPrint', $sanad->id) }}" target="_blank" data-toggle="tooltip" data-placement="bottom" title="طباعة">
