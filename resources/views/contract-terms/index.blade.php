@extends('const.const')
@section('content')
<div class="col-12  row" style="padding: 8px;margin-top: 55px">
    <div class="panel panel-default   col-12 ">
      <div class="panel-heading  col-12" style="padding: 0px; ">
            <div class="panel-body col-12 row " style=" background: #2381c6;color: #fff;padding:  20px 5px; ">
                <div class="col-6 " style="padding: 4px">
                    <span class="fa fa-user" style="color: var(--orange-light-color);margin-right: 10px;padding-left: 20px;font-size: 22px;"></span>  بيانات العقد
                </div>
                @permission('create-contractTerm')
                <div class="col-6  text-right" style="padding: 0px;display: unset;direction: ltr">
                    <button></button>
                    <a href="{{ route('contract-terms.create') }}">
                         <button style="border:none;border-radius: 40px!important;padding: 10px 25px;background: var(--orange-color);display: inline-block;color: #fff;float: left;">إضافة</button>
                    </a>
                </div>
                @endpermission
            </div>
        </div>

            <div class="row mt-3 mb-3">
                <div class="col-md-6 col-xs-12">
                    <form action="{{ route('contracts.start.update', $contractStart->id) }}" method="post">
                        @csrf
                        @method('put')
                        <input type="hidden" name="type" value="start">
                        <label for="start">مقدمة العقد</label>
                        <textarea class="form-control" name="value" id="start" cols="30" rows="8">{{ $contractStart->value }}</textarea>
                        <button type="submit" class="btn btn-primary mt-2">حفظ</button>
                    </form>
                </div>
                <div class="col-md-6 col-xs-12">
                    <form action="{{ route('contracts.end.update', $contractEnd->id) }}" method="post">
                        @csrf
                        @method('put')
                        <input type="hidden" name="type" value="end">
                        <label for="end">خاتمة العقد</label>
                        <textarea class="form-control" name="value" id="end" cols="30" rows="8">{{ $contractEnd->value }}</textarea>
                        <button type="submit" class="btn btn-primary mt-2">حفظ</button>
                    </form>
                </div>
            </div>
            <hr>
            <div class="panel-body row" style="padding: 30px 0px;  ">

               @include('const.enternalConst.alert-remove')
               @include('const.enternalConst.loading-img')

                <div class="col-12 real-content row " style="opacity: 0;padding:30px  0px;position: relative;">
                    <table  id="myTable" class="table table-striped table-bordered col-12 " style="padding: 0px;">
                        <thead>
                            <tr>
                                <th>الكود</th>
                                <th>البند</th>
                                @permission(['update-contractTerm', 'delete-contractTerm'])
                                <th>عملية</th>
                                @endpermission
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($contractTerms as $contractTerm)
                            <tr>
                                <td>{{$contractTerm->id}}</td>
                                <td>{{$contractTerm->term}}</td>

                                @permission(['update-contractTerm', 'delete-contractTerm'])
                                <td style="max-width: 100px">
                                    @permission('update-contractTerm')
                                    <a href='{{ route('contract-terms.edit', $contractTerm->id) }}'  data-toggle="tooltip" data-placement="bottom" title="تعديل">
                                        <span class="far fa-edit"></span>
                                    </a>
                                    @endpermission
                                    @permission('delete-contractTerm')
                                    <a href="#" onclick="
                                            $('.alert-layer').fadeIn();
                                            $('#remove-target').attr('action','{{ route('contract-terms.destroy', $contractTerm->id) }}');
                                          "  data-toggle="tooltip" data-placement="bottom" title="حذف">
                                        <span class="far fa-trash-alt"></span>
                                    </a>
                                    @endpermission
                                </td>
                                @endpermission
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>


        </div>
</div>
@endsection
