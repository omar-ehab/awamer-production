@extends('const.const')
@section('content')
    <div class="col-12  row" style="padding: 8px;margin-top: 50px">
            ...
            <div class="panel-heading  col-12" style="padding: 0px; ">
                <div class="panel-body col-12 row " style=" background: #2381c6;color: #fff;padding:  20px 5px; ">
                    <div class="col-6 " style="padding: 4px">
                        <span class="fa fa-clock" style="color: var(--orange-light-color);margin-right: 10px;padding-left: 20px;font-size: 22px;"></span>  الفترات
                    </div>
                    @permission('create-interval')
                ...
                <div class="col-12 real-content row " style="opacity: 0;padding: 30px 0px;position: relative;">
                    <table  id="myTable" class="table table-striped table-bordered col-12 " style="padding: 0px;">
                        ...
                        <tr>
                            ...
                            <th>من</th>
                            <th>إلي</th>
                            <th>عدد الأيام بالأرقام</th>
                            @permission(['update-interval', 'delete-interval'])
                            <th>عملية</th>
                            @endpermission
                        </tr>
                        </thead>
                        ...
                        @foreach($intervals as $interval)
                            <tr>
                                ...
                                <td>{{$interval->end}}</td>
                                <td>{{\Carbon\Carbon::parse($interval->end)->diffInDays(\Carbon\Carbon::parse($interval->start))}}</td>
                                @permission(['update-interval', 'delete-interval'])
                                <td style="max-width: 100px">
                                    @permission('update-interval')
                                    <a href='{{ route('intervals.edit', $interval->id) }}'  data-toggle="tooltip" data-placement="bottom" title="تعديل">
                                        <span class="far fa-edit"></span>
                                    </a>
                                    @endpermission
                                    @permission('delete-interval')
                                    <a href="#" onclick="
                                        $('.alert-layer').fadeIn();
                                        $('#remove-target').attr('action','{{ route('intervals.destroy', $interval->id) }}');
